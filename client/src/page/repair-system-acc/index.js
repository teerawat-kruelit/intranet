  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        let itResp = await axios.get('http://localhost:4000/api/repair_list_acc/it', { withCredentials: true })
        if (itResp?.data?.status) {
          setItData(itResp.data.data)
        }
        let buildingtResp = await axios.get('http://localhost:4000/api/repair_list_acc/building', { withCredentials: true })
